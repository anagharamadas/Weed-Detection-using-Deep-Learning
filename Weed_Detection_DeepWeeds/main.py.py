...
IMG_DIRECTORY = "C:/Users/sivad/Documents/Gal/DeepWeeds-master/images/"
...
def cross_validate(model_name):
    ...
    for k in range(FOLDS):
        ...
        if model_name == "agroavnet":
            ...
            x = Dropout(0.8)(x)
            x = Dense(units=4096, activation='relu', kernel_regularizer=regularizers.l2(0.01))(x)
        ...
        # Assemble the modified model
            ...
            x = model(input, training=False)
            clf = svm.SVC.fit(x)
            outputs = Dense(9, activation='sigmoid', name='fc9')(clf)
            model = Model(inputs=input, outputs=outputs)
...
if __name__ == '__main__':
    ...
    if command == "cross_validate":
        #if not model == "resnet" and not model == "inceptionresnet":
            #print("Error: You must ask for either ""resnet"" or ""inceptionresnet"".")
        #else:
